...
public class FrozenRepositoryImplTest {
  ...
  public void checkFrozenAlerts() {
    final Set<String> members = new HashSet<String>(Arrays.asList("providerTest:sensorTest:alertTest1", "providerTest:sensorTest:alertTest2"));
  ...
  public void synchronizeAlerts() {
    ...
    when(alert.getId()).thenReturn(alertId);
    when(alert.getExpression()).thenReturn("20");
    when(alert.getUpdateAt()).thenReturn(new Date(5l)).thenReturn(new Date(1l));
